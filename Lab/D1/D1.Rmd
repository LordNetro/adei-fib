...
#######################################################
imis<-rep(0,nrow(dfVariables))  # rows - trips
jmis<-rep(0,ncol(dfVariables))  # columns - variables
######################################################
mis1<-countNA(dfVariables)
imis<-mis1$mis_ind
mis1$mis_col # Number of missings for the current set of variables

#######################################################
iouts<-rep(0,nrow(dfVariables))  # rows - trips
jouts<-rep(0,ncol(dfVariables))  # columns - variables
######################################################

#######################################################
ierrs<-rep(0,nrow(dfVariables))  # rows - trips
jerrs<-rep(0,ncol(dfVariables))  # columns - variables
######################################################

dfVariables[which(dfVariables[,"engineSize"]==0),]

# EngineSize
# It is a quantitive variable  Non-possible values will be recoded to NA
selEngine<-which(dfVariables$engineSize ==0)
...
```{r}
boxplot(dfVariables$price, main="Boxplot of Price", ylab="Price")
var_outPrice<-calcQ(dfVariables$price)
summary(dfVariables$price)
abline(h=var_outPrice$souts,col="red")
abline(h=var_outPrice$souti,col="red")

lloutPrice<-which((dfVariables$price<var_outPrice$souti)|(dfVariables$price>var_outPrice$souts))
iouts[lloutPrice]<-iouts[lloutPrice]+1
jouts[1]<-length(lloutPrice)

boxplot(dfVariables$engineSize, main="Boxplot of Engine Size", ylab="Engine Size")
var_outEngine<-calcQ(dfVariables$engineSize)
summary(dfVariables$engineSize)
abline(h=var_outEngine$souts,col="red")
abline(h=var_outEngine$souti,col="red")

lloutEngine<-which((dfVariables$price<var_outEngine$souti)|(dfVariables$price>var_outEngine$souts))
